...
<template>
  <aside v-if="isLoggedIn" :class="isExpanded ? 'w-64' : 'w-20'" class="bg-[#0f172a] text-white transition-all duration-300 flex flex-col min-h-screen shadow-2xl relative border-r border-slate-800">
    ...
    <nav class="flex-1 px-4 py-6 space-y-3">
      ...
      <router-link v-if="['Admin','Director','Docente'].includes(userRol)" to="/tareas" title="Seguimiento de tareas" class="flex items-center px-3 py-3 rounded-xl text-slate-300 hover:bg-slate-800" active-class="bg-blue-600 !text-white" :class="isExpanded ? '' : 'justify-center'"><span class="w-6 h-6 border border-current rounded text-center text-sm leading-6 shrink-0" aria-hidden="true">T</span><span v-if="isExpanded" class="ml-4 font-medium">Seguimiento de tareas</span><span v-else class="sr-only">Seguimiento de tareas</span></router-link>
    </nav>
